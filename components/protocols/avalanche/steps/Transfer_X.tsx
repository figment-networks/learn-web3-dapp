...
import { getAvalancheClient, getAvalancheExplorerURL } from '@avax/utils'
import {
  AvalancheKeypairType,
  AvalancheTransferResponse,
  AvalancheTransferErrorResponse
} from '@avax/types'
...
const Transfer = ({ keypair }: { keypair: AvalancheKeypairType | null }) => {
  ...
  if (!keypair)
    return <Alert type='error' showIcon message={'Generate a Keypair first'} />

  const generateKeypair = () => {
    const address = getAvalancheClient()
      .XChain()
      .keyChain()
      .makeKey()
      .getAddressString()
  ...
  const transfer = (values: any) => {
    ...
    axios
      .post(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/avalanche/transfer`, {
  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        name='amount'
        required
        tooltip='1 billion nAVAX = 1 AVAX'
      >
      ...
      {txId && (
        <Form.Item {...tailLayout}>
          <Alert
            ...
            description={
              <a
                href={getAvalancheExplorerURL(txId)}
                target='_blank'
                rel='noreferrer'
              >
